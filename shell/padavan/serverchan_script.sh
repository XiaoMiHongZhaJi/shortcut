...
if [ ! -z "$ping_time" ] ; then
echo "online"
#logger -t "【微信推送】" "online"
if [ "$serverchan_notify_1" = "1" ] ; then
    hostIP=$(arIpAddress)
    #logger -t "【微信推送】" "arIpAddress: ${hostIP}"
    hostIP=`echo $hostIP | head -n1 | cut -d' ' -f1`
    if [ "$hostIP"x = "x"  ] ; then
        curltest=`which curl`
        if [ -z "$curltest" ] || [ ! -s "`which curl`" ] ; then
            [ "$hostIP"x = "x"  ] && hostIP=`wget -T 5 -t 3 --user-agent "$user_agent" --quiet --output-document=- "http://checkipv6.dyndns.com" | grep Address | head -n1 | cut -d':' -f2- | cut -d' ' -f2 | cut -d'<' -f1`
            ...
            [ "$hostIP"x = "x"  ] && hostIP=`curl -L --user-agent "$user_agent" -s http://checkipv6.dyndns.com | grep Address | head -n1 | cut -d':' -f2- | cut -d' ' -f2 | cut -d'<' -f1`
        ...
        #logger -t "【微信推送】" "为空，再次判断arIpAddress: ${hostIP}"
    fi
    lastIP=$(lastIPAddress)
    #logger -t "【微信推送】" "lastIPAddress: ${lastIP}"
    if [ "$lastIP" != "$hostIP" ] && [ ! -z "$hostIP" ] ; then
    sleep 60
        hostIP=$(arIpAddress)
        #logger -t "【微信推送】" "60秒后再次判断arIpAddress: ${hostIP}"
        hostIP=`echo $hostIP | head -n1 | cut -d' ' -f1`
        lastIP=$(lastIPAddress)
    fi
    if [ "$lastIP" != "$hostIP" ] && [ ! -z "$hostIP" ] ; then
        ...
        #url1="http://sctapi.ftqq.com/$serverchan_sckey.send?text=【PDCN_"`nvram get computer_name`"】互联网IP变动：${hostIP}"
        #curl -L -s ${url1} -d "&desp=${hostIP}" &
...
if [ "$serverchan_notify_2" = "1" ] ; then
    ...
    if [ -s "/tmp/var/newhostname不重复.txt" ] ; then
        ...
        #url1="http://sctapi.ftqq.com/$serverchan_sckey.send?text=【PDCN_"`nvram get computer_name`"】新设备加入：${content}"
        #curl -L -s ${url1} -d "&desp=${content}" &
...
if [ "$serverchan_notify_4" = "1" ] ; then
    ...
    if [ -s "/tmp/var/newhostname不重复_上线.txt" ] ; then
        ...
        #url1="http://sctapi.ftqq.com/$serverchan_sckey.send?text=【PDCN_"`nvram get computer_name`"】设备【上线】Online：${content}"
        #curl -L -s ${url1} -d "&desp=${content}" &
    ...
    if [ -s "/tmp/var/newhostname不重复_下线.txt" ] ; then
        ...
        #url1="http://sctapi.ftqq.com/$serverchan_sckey.send?text=【PDCN_"`nvram get computer_name`"】设备【下线】offline：${content}"
        #curl -L -s ${url1} -d "&desp=${content}" &
...
if [ "$serverchan_notify_3" = "1" ] && [ "$resub" = "1" ] ; then
    ...
    if [ "$(cat /tmp/var/osub |head -n1)"x != "$(cat /tmp/var/nsub |head -n1)"x ] && [ -f /tmp/var/nsub ] ; then
        ...
        #url1="http://sctapi.ftqq.com/$serverchan_sckey.send?text=【PDCN_"`nvram get computer_name`"】固件更新提醒：${content}"
        #curl -L -s ${url1} -d "&desp=${content}" &
